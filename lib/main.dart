...
<body class="bg-white text-gray-900">
    <div class="h-screen flex flex-col">
        ...
        <div id="main-scroll">
            <div class="px-4 pt-3">
                <div class="bg-gray-100 rounded-lg p-0.5 flex">
                    <button id="btn-candles" class="flex-1 py-2 rounded-md text-sm font-semibold transition-colors bg-white text-gray-900 shadow-sm">Candles</button>
                    <button id="btn-line" class="flex-1 py-2 rounded-md text-sm font-semibold transition-colors text-gray-500">Line</button>
                </div>
            </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 flex gap-3 z-10">
            <button id="btn-rise" class="flex-1 bg-green-500 py-4 rounded-xl font-bold text-white btn-active">Rise</button>
            <button id="btn-fall" class="flex-1 bg-red-500 py-4 rounded-xl font-bold text-white btn-active">Fall</button>
        </div>
            ...
            <div class="modal-enter absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[70vh] shadow-2xl">
                ...
                <div class="p-4 overflow-y-auto" style="max-height: calc(70vh - 120px);">
                    <div class="mb-4">
                        ...
                        <input type="number" id="stake-input" value="1.00" step="0.01" min="0.35" class="w-full bg-gray-100 px-4 py-3 rounded-xl text-gray-900 border border-gray-200">
                    </div>

                    <div class="mb-4">
                        <label class="text-sm font-semibold text-gray-600 mb-2 block">Duração</label>
                        <div class="flex gap-2">
                            <input type="number" id="duration-input" value="5" min="1" class="flex-1 bg-gray-100 px-4 py-3 rounded-xl text-gray-900 border border-gray-200">
                            <div class="bg-gray-100 rounded-xl flex p-1 border border-gray-200">
                                <button data-unit="t" class="px-3 py-2 rounded-lg text-sm font-semibold btn-active duration-unit bg-green-500 text-white">Ticks</button>
                                <button data-unit="s" class="px-3 py-2 rounded-lg text-sm font-semibold btn-active duration-unit text-gray-500">Seg</button>
                                <button data-unit="m" class="px-3 py-2 rounded-lg text-sm font-semibold btn-active duration-unit text-gray-500">Min</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pb-3">
                        ...
                        <button id="execute-trade" class="hidden w-full rounded-xl font-bold text-white btn-active">
    ...
    <script>
        ...
        const state = {
            ...
            balance: 232.14, currency: 'USD', chartMode: 'candles', selectedTimeframe: 0,
            contractType: 'CALL', proposalId: null, durationUnit: 't', candles: [], ticks: [], proposalTimeout: null
        };
        ...
        function initChart() {
            ...
            state.areaSeries = state.chart.addAreaSeries({
                topColor: 'rgba(16, 185, 129, 0.4)', bottomColor: 'rgba(16, 185, 129, 0)',
        ...
        function connectChartWebSocket() {
            ...
            state.chartWs.onopen = () => subscribeToChartData();
        ...
        function subscribeToChartData() {
            ...
            if (state.selectedTimeframe === 0) {
                state.chartWs.send(JSON.stringify({ ticks: MARKET_SYMBOL, subscribe: 1 }));
            } else {
                const now = Math.floor(Date.now() / 1000);
                const start = now - (state.selectedTimeframe * 1000);
                state.chartWs.send(JSON.stringify({
                    ticks_history: MARKET_SYMBOL, adjust_start_time: 1, count: 1000,
                    end: 'latest', start: start, style: 'candles', granularity: state.selectedTimeframe
                }));
                state.chartWs.send(JSON.stringify({
                    ticks_history: MARKET_SYMBOL, adjust_start_time: 1, count: 1, end: 'latest',
                    start: 1, style: 'candles', granularity: state.selectedTimeframe, subscribe: 1
                }));
            }
        }
        ...
        function handleChartData(data) {
            if (data.msg_type === 'history' && data.candles) {
                ...
                state.ticks.push({ time: data.tick.epoch, value: price });
                if (state.chartMode === 'line' && state.selectedTimeframe === 0) {
                    state.areaSeries.update({ time: data.tick.epoch, value: price });
                }
            } else if (data.msg_type === 'ohlc') {
                const candle = {
                    time: data.ohlc.epoch, open: parseFloat(data.ohlc.open),
                    high: parseFloat(data.ohlc.high), low: parseFloat(data.ohlc.low),
                ...
                if (state.chartMode === 'candles') state.candleSeries.update(candle);
        ...
        function updateChart() {
            if (state.chartMode === 'candles') {
                ...
                if (state.candles.length > 0) state.candleSeries.setData(state.candles);
            } else {
        ...
        function openTradeModal(type) {
            ...
            el.executeTradeBtn.className = `w-full rounded-xl font-bold text-white btn-active ${color}`;
            el.durationUnits.forEach(btn => {
        ...
        function requestProposal() {
            ...
            state.ws.send(JSON.stringify({
                proposal: 1, amount: parseFloat(el.stakeInput.value) || 1.0, basis: 'stake',
                contract_type: state.contractType, currency: state.currency,
                duration: parseInt(el.durationInput.value) || 5,
                duration_unit: state.durationUnit, symbol: MARKET_SYMBOL
            }));
        }
