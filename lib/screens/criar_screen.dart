...
const _svgDocumento = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.656.93,6.464,13.122A4.966,4.966,0,0,0,5,16.657V18a1,1,0,0,0,1,1H7.343a4.966,4.966,0,0,0,3.535-1.464L23.07,5.344a3.125,3.125,0,0,0,0-4.414A3.194,3.194,0,0,0,18.656.93Zm3,3L9.464,16.122A3.02,3.02,0,0,1,7.343,17H7v-.343a3.02,3.02,0,0,1,.878-2.121L20.07,2.344a1.148,1.148,0,0,1,1.586,0A1.123,1.123,0,0,1,21.656,3.93Z"/><path d="M23,8.979a1,1,0,0,0-1,1V15H18a3,3,0,0,0-3,3v4H5a3,3,0,0,1-3-3V5A3,3,0,0,1,5,2h9.042a1,1,0,0,0,0-2H5A5.006,5.006,0,0,0,0,5V19a5.006,5.006,0,0,0,5,5H16.343a4.968,4.968,0,0,0,3.536-1.464l2.656-2.658A4.968,4.968,0,0,0,24,16.343V9.979A1,1,0,0,0,23,8.979ZM18.465,21.122a2.975,2.975,0,0,1-1.465.8V18a1,1,0,0,1,1-1h3.925a3.016,3.016,0,0,1-.8,1.464Z"/></svg>';
...
Widget _svg(String d, Color c, {double s = 22}) => SvgPicture.string(d, width: s, height: s, colorFilter: ColorFilter.mode(c, BlendMode.srcIn));
    ...
    return Scaffold(
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Text('ESCOLHE UM TIPO', style: GoogleFonts.syne(color: textSec, fontSize: 11, fontWeight: FontWeight.w600, letterSpacing: 1.2)),
        ),
        Expanded(child: ListView(padding: EdgeInsets.zero, children: [
          _Item(svg: _svgDocumento, acc: acc, isDark: isDark, title: 'Documento', subtitle: 'Texto com formatação rica',
            onTap: () async {
              await Navigator.push(context, MaterialPageRoute(builder: (_) => const EditorScreen()));
              widget.onDocCreated?.call();
            }),
          _Item(svg: _svgApresentacao, acc: acc, isDark: isDark, title: 'Apresentação', subtitle: 'Em breve', disabled: true),
          _Item(svg: _svgCV, acc: acc, isDark: isDark, title: 'Currículo / CV', subtitle: 'Em breve', disabled: true),
          _Item(svg: _svgNota, acc: acc, isDark: isDark, title: 'Nota', subtitle: 'Em breve', disabled: true),
          const SizedBox(height: 8),
          Padding(
            padding: const EdgeInsets.fromLTRB(20, 16, 20, 8),
            child: Text('IMPORTAR', style: GoogleFonts.syne(color: textSec, fontSize: 11, fontWeight: FontWeight.w600, letterSpacing: 1.2)),
          ),
          _Item(svg: _svgCarregar, acc: acc, isDark: isDark, title: 'Carregar ficheiro', subtitle: 'PDF, DOCX, TXT e mais', onTap: _carregar),
        ])),
      ]),
    );
  }
...
class _Item extends StatelessWidget {
  final String svg; final Color acc; final bool isDark;
  final String title; final String subtitle;
  final bool disabled; final VoidCallback? onTap;
  const _Item({required this.svg, required this.acc, required this.isDark, required this.title, required this.subtitle, this.disabled = false, this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final iconBg    = disabled ? (isDark ? AppColors.darkSurface : const Color(0xFFF5F5F5)) : acc.withOpacity(.1);
    return Opacity(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: bg, border: Border(bottom: BorderSide(color: divColor, width: 0.5))),
          child: Row(children: [
            // CIRCULAR container
            Container(width: 46, height: 46, decoration: BoxDecoration(color: iconBg, shape: BoxShape.circle), child: Center(child: _svg(svg, iconColor, s: 24))),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, style: GoogleFonts.syne(color: textPrimary, fontWeight: FontWeight.w700, fontSize: 15)),
              const SizedBox(height: 3),
              Text(subtitle, style: GoogleFonts.syne(color: textSec, fontSize: 13)),
            ])),
